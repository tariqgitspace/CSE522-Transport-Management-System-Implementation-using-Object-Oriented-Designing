	...
	public void run() {
		...
		for (int i = 0; i < s1.route.routes.size(); i++) {
			...
			//System.out.println("Bus: " + b1.licensePlateNo + " being driven by: " + d1.drivername + " is at stop: "
				//	+ b1.returnLocation().name);
			
			b1.updateLocation(s1.route.routes.get(i));

			
			try {
				Thread.sleep(2000);  //Interval at which to Stop change
			} catch (InterruptedException e) {
				// TODO Auto-generated catch block
				e.printStackTrace();
			}
		...
		//Code Added (To Clear Console for Better Demo)
		for(int clear = 0; clear < 1000; clear++)
		  {
		     //System.out.println("") ;
		  }
		
		//System.out.println("Trip Ended For Bus: " + b1.licensePlateNo );
...
class maintenanceStaff implements employees {
	...
	public void checkMaintenanceStatus() {
		...
		for (int i = 0; i < number_of_buses; i++) {
			if (temp[i].getTripCount() < maintainance_Threshold) {
				...
				System.out.println(temp[i].getTripCount() +" Maintenance needed For Bus: #" + temp[i].licensePlateNo + " .Remove from available List ");
				centralControlSystem.removeVehicleFromAvailableList(i);
				SendVehicletoGarage(i,temp[i].licensePlateNo);
			}
		}
	}
	...
	public void SendVehicletoGarage(Integer vehicle_number, Integer plate_number) {

		// Assume vehicle is in garage for repair for 3 seconds
		try {
			Thread.sleep(5000);  //In Garage
		...
		System.out.println(" Maintenance Over for Bus: " + plate_number + ". Add Bus again to Avaialble List ");
	}

	Integer maintainance_Threshold = 7;
}
...
class vanRequestService {
	...
	while(iter.hasNext()){
	iter.next().displayNotifications("Notification:   Van is being deployed for stop: "+s1.name);
	}
...
class commuters {
	...
	public commuters(int id) {
		this.id = id;
	//	System.out.println("Commuter Registration Complete");
	}
	...
	//Code Added
	public static boolean authoriseCommuter(int Id) {

		boolean result=false;
		for(int i=0;i<centralControlSystem.commuter_database.size() ;i++)
		{
			if(Id==centralControlSystem.commuter_database.get(i).id)
			{
				System.out.println("User Is Authorised to Board Bus");
				return true;
			}
		}
	...
	public void retrieveRouteMap(String sourceStop, String destStop) {
		...
		this.displayNotifications("User: "+ id+" Notification : "+commuterDisplay);
	}
...
class centralControlSystem {
	...
	public centralControlSystem() throws InterruptedException {
		...
		//Code Added (Initial and Final Destination of all Buses is dockyard)
		route[0] = new Route(new Stops[] {stops[8], stops[3], stops[6], stops[7],stops[8] }, "Yellow Line");
		route[1] = new Route(new Stops[] { stops[8],stops[7], stops[6], stops[3],stops[8] }, "Green Line");
		route[2] = new Route(new Stops[] { stops[8],stops[0], stops[1], stops[2], stops[3], stops[4], stops[5], stops[7],stops[8] },
				"South Campus");
		route[3] = new Route(new Stops[] { stops[8], stops[7], stops[5], stops[4], stops[3], stops[2], stops[1], stops[0],stops[8] },
				"North Campus");
	...
	public static void AddScheduleForOverCrowdRequest(Route request, double time) {
		...
		//System.out.println("AddScheduleForOverCrowdRequest");
		...
		String commuterDisplay = "New Buses added at route " + request.name + " at "+
				schedule[15].starttime;
		
		//Code Added
		for(int i=0;i<commuter_database.size();i++)
		{
			commuter_database.get(i).displayNotifications("User: "+ commuter_database.get(i).id+" Notification : "+commuterDisplay);
		}
	}
...
public class BusTransportSystem {
	public static void main(String[] args) throws InterruptedException, IOException {
	    ...
	    commuters c1 = null,c2=null;
	    new centralControlSystem();
	    Route r=null;
	    
	    
	    while(true){
	        String Use_Case = into.readLine();
	        switch(Integer.parseInt(Use_Case)){

	            case 1:
		           System.out.println("Register 1st Commuter");
		           String Commuter_Id = into.readLine();
	               int id = Integer.parseInt(Commuter_Id);
	               c1=new commuters(id);
	               c1.registerCommuter();
		           System.out.println("Register 2nd Commuter");
		           Commuter_Id = into.readLine();
	               id = Integer.parseInt(Commuter_Id);
	               c2=new commuters(id);
	               c2.registerCommuter();
	               System.out.println("Commuter Registration Complete");
		      		break;
	            ...
	            case 4:
	        		c1.retrieveRealTimeLocation();
	        		c2.retrieveRealTimeLocation();
	        		break;
	        	
	            case 5:  //Use Case 5 and 7
	            	r = centralControlSystem.route[0];
	            	Driver.PlyadditionalBus(r , 3);
	        		//c1.retrieveRealTimeLocation();
	            	break;
	            case 6:
	            	System.out.println("Already Running in BackGround");
	            	break;
	            	
	            case 10:
			           System.out.println("Check Commuter Authorisation");
			           Commuter_Id = into.readLine();
		               id = Integer.parseInt(Commuter_Id);
		               commuters.authoriseCommuter(id);
	            	break;      	
